...
def leverage_cur(df: pd.DataFrame, sing_vect_num: int, search_size=1) -> tuple[np.array, np.array, np.array, dict]:
    ...
    cur_dist = np.linalg.norm(df.to_numpy() - col_matrix.dot(u_matrix).dot(row_matrix))
    
    return col_matrix, u_matrix, row_matrix, {"cur_dist": cur_dist, "col_dist": col_distance, "row_dist": row_distance}